...
| Commit | What landed |
|---|---|
| `bb8b39b` | **BL-67 tier 1 — the co-founder initiates.** The hub's "one thing to settle" card asks the first unresolved required arc intent (real question, owning chat, no model call); the founder's answer posts to `/api/settle` and lands as an intent-tagged memory, so coverage and signals move and the chat won't re-ask. Ownership + intent-belongs-to-this-arc guarded; atomic save with activity row; "Not now" dismisses; card vanishes when nothing is unresolved. Model-generated chips deliberately deferred to tier 2 |
| `4ad093d` | **BL-72 stage 2 — the hatch ceremony and the egg loader language.** Onboarding's emoji egg becomes the four-scene ceremony: answers absorbed into the egg, conversations fanning out, brief seeding, double crack and hatch — continuous by construction (every prop exits, egg loops never restart, cracks draw by transition, motes throughout). Holds its last scene until `/api/ideas` actually returns, so the payoff is never cut off and the wait is never faked; reduced motion lands on the finished state. Egg dots replace the chat's typing dots; a rocking egg replaces the Send label mid-turn |
| `a2bd5ef` | **BL-71 + BL-72 stage 1 — the hub wears the new design.** Signals widget (Option K) replaces the dot grid: absolute bars toward 4-memories-is-explored, filling egglets, untouched topics visible, plain-language latest line that follows the hovered row. Stat row (gate · intents · memories · open questions) above the conversations; rail reordered brief → signals → memory → artifacts; tagline moved to its own line. Every chat card keeps its description alongside coverage, and the collapsed deck reads as a drawer naming the chat you're in with a pip per chat. Typecheck/lint/build clean; **hub is auth-gated so visual confirmation is the user's** |
| `96c510d` + `e5ceb83` | **BL-65 + BL-62 + BL-66 — the corruption containment package**, from the extension-test triage. Turn integrity: all four observed corruption signatures (brace floods, serialization fragments, spaced + concatenated stutter) scanned across every model-returned string — reply, memory content/verbatim, brief updates, chips, naming — any hit = degenerate → retry → graceful; husk replies degenerate regardless of extraction. Spiral burn bounded: max_tokens 16k→6k, client abort 60s→120s. Contradictions re-tag their intent node — conflict narration never lives untagged (stale Prepped row superseded in place). Detectors probed 10/10 against production strings; guard pair clean and the day's two fastest runs (339s, 280s) |
| `8094ff8` | **BL-60 — dead-end chats, found by Dino, fixed same day.** Two halves: intents answered by existing context (idea already named; one-liner as pitch) were treated as resolved but never tagged, stranding coverage at 2/3 forever; and a chat the model judged settled ended with no question, no chips, no pointer. Now: resolution must be recorded ("never just note it already landed"), and every reply ends with a next move — next intent, one deepening question, or a handoff chip to the chat owning the next gap. Existing stuck chats self-heal on the next message. Guard pair committed |
