...
| BL-67 | **Decision prompts — the co-founder initiates** | M | todo | From Dino 2026-08-01. One-way prompts from the idea's memory ("the conscience"): a single card that asks the one thing worth settling, founder taps or types an answer, no rebuttal loop. Tier 1 (mechanical, zero model calls): unresolved required intents + brief open questions, answer writes an intent-tagged memory — coverage/brief machinery reused whole. Tier 2 (conscience pass, one small call): pick the highest-leverage question from assumptions to test, contradictions, `question`-kind memories, zero-signal topics. Escape hatch: "take it to the chat" deep-link. Load-bearing constraint: **never nagging** — one at a time, dismissible, "not now" sticks. Pairs with BL-61 (multi-select). Sprint 7 headline candidate |
