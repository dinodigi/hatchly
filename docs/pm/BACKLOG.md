...
| BL-65 | **Turn-integrity guard — corruption hits ANY string field, not just replies** | M | done | `96c510d` 2026-08-01. All four signatures detected across every model-returned string; husk replies degenerate regardless of extraction. Probed 10/10 vs real production strings; guard pair clean (0 and 1 raw degenerates, none past retry, no retry storm) | Extension-test verdict + DB verification 2026-08-01: garbled replies, the ":" husk, AND a memory whose `content` was a corrupted sample (which the Activity old→new trail then faithfully displayed — the "leak" was honest rendering of poisoned data; "5 meals/week"'s clean capture was the casualty). Detect corruption signatures — serialization fragments (`'kind':` etc.), brace runs, token stutter — across reply + memory contents + brief updates; any hit = degenerate turn → BL-01 retry. Subsumes the ":"-with-extraction guard gap (short reply + successful extraction currently bypasses retry) |
| BL-66 | **Contradiction captures must ride the intent node — stale conflict narration re-litigates settled decisions** | S | done | Prompt rule shipped in `e5ceb83` (rode the BL-62 commit — noted for attribution); the stale Prepped conflict row superseded in place. Fixture can't exercise contradictions — verified by instruction review; watch the next real conversation | Verified: an UNTAGGED row "reconsiders $25/meal, conflicting with the locked-in $12/meal" survived next to the resolved `price` node; First 100 users read it and reported the conflict "still open" — factually wrong per the same memory. Interim reconsiderations must re-tag the intent (singular update keeps history in Activity); conflict narration never lives untagged. Prompt iteration + consider a supersede sweep when a node resolves |
...
| BL-72 | **Idea Hub redesign — per the vision artifact** | L | todo | Clickable final-product mock (approved pending Dino's click-through): slim one-line masthead, stat strip, Signal Board as an EGG PIE (Dino's call, 2026-08-01: ten repeated eggs read as wallpaper) — one radial chart, ten fixed wedges filling outward with signal, dashed shells for untouched topics, single egg at the hub, provenance detail panel beside it, no nested sidebar scrolling, decision-prompt card (BL-67's surface), grand hatch onboarding (~15s ceremony, rotating quotes, always skippable), egg loading system (thinking/busy/hatching/empty). Artifact: claude.ai/code/artifact/d6d9d2c1-a4cf-4ec9-a4bb-ba6cfbeae0fa — break into build items once direction is approved |
