...
| BL-62 | **Bound the degenerate-spiral burn — cap `max_tokens`** | S | todo | From the 60s "took too long" during Dino's extension test (2026-08-01). A brace-spiral generates to the 16k cap (~1min+) before the BL-59 guard can discard it, then the retry doubles model time — blowing the 60s client abort while the server still completes. A legit turn is 1–2k tokens; cap at ~4k to quarter the spiral cost. Consider nudging ChatPanel's 60s timeout. NOT Pluggie: `[mcp]` logs hold at 150–500ms. Real perceived-speed fix stays BL-11 (streaming). Apply after the extension test run finishes |
