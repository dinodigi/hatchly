...
| BL-63 | **Cross-chat facts don't get re-tagged for the current chat's arc — badges stall while the model declares "settled"** | S | todo | Verified: market chat missing tagged `beachhead`/`competitors` (competitor facts captured untagged in What you're building); gtm missing `watering_holes`; both chats declared settled in text. BL-60's rule fires partially — needs an iteration, or a mechanical assist (match untagged memories to open intents server-side) |
| BL-64 | **Onboarding placeholder survives a completed problem chat** | S | todo | Verified: `brief.problem` = "Too slow, manual, or painful" (the onboarding pick, verbatim) on a ✓-covered chat with 4 rich problem memories. A non-empty placeholder reads as "filled". Mark onboarding drafts replaceable (e.g. "Draft:" prefix like `who` uses) and make the problem chat's GOOD condition include writing the confirmed problem to the brief |
