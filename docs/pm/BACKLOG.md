...
| BL-65 | **Turn-integrity guard — corruption hits ANY string field, not just replies** | M | done | `96c510d` 2026-08-01. All four signatures detected across every model-returned string; husk replies degenerate regardless of extraction. Probed 10/10 vs real production strings; guard pair clean (0 and 1 raw degenerates, none past retry, no retry storm). Origin: extension-test verdict + DB verification — corrupted memory content is how garbage reached the Activity trail |
| BL-66 | **Contradiction captures must ride the intent node — stale conflict narration re-litigates settled decisions** | S | done | Prompt rule shipped in `e5ceb83` (rode the BL-62 commit — noted for attribution); the stale Prepped conflict row superseded in place. Fixture can't exercise contradictions — verified by instruction review, then PASSED in the re-test (never re-litigated). Origin: untagged "conflicting with" row survived beside the resolved `price` node |
| BL-63 | **Cross-chat facts don't get re-tagged for the current chat's arc — badges stall while the model declares "settled"** | S | done | `d7ff11a` 2026-08-01 (second iteration on BL-60): settled-declaration now tied to the tag ledger — confirming-and-tagging an unmarked required intent IS the next move. Guard run 1 hit **2/12 no-intent, best ever and under target**. Mechanical server-side assist remains the fallback if a third iteration is ever needed |
| BL-64 | **Onboarding placeholder survives a completed problem chat** | S | done | `158bdcf` 2026-08-01. Onboarding seeds `problem` as "Draft: …"; the brief instruction replaces Draft-sections with confirmed text (dropping the prefix, never writing one). Note: the fixture starts pre-onboarding-brief, so the seed path's end-to-end proof is the next fresh-idea test |
| BL-62 | **Bound the degenerate-spiral burn — cap `max_tokens`** | S | done | `e5ceb83` 2026-08-01. Cap 16k→6k (legit fat turns peak ~4k — headroom kept so truncation can't fake a degenerate), client abort 60s→120s so spiral+retry turns resolve in-window instead of showing "took too long" while succeeding. Guard pair ran 339s and **280s — the day's fastest** vs the 316–602s pre-cap band. Real perceived-speed fix stays BL-11 (streaming) |
...
| BL-71 | **Chat-strip fixes: hover clipping + missing descriptions** | S | done | `a2bd5ef` 2026-08-01. Cards now carry description AND coverage (was either/or); collapsed deck reads as a drawer with pips. | From Dino's UI review 2026-08-01: cards clip when they lift on hover (strip container overflow), and not every card shows its subtitle. The vision mock demonstrates the fix (vertical breathing room in the strip, `overflow-x` only, description clamped to 2 lines on every card) |
| BL-72 | **Idea Hub redesign — per the vision artifact** | L | doing | **Stages 1-3 shipped** (`a2bd5ef` widgets, `4ad093d` hatch + egg loaders, `bb8b39b` settle card). Remaining: the artifact's two-column dashboard arrangement (app keeps stats-above-conversations, closer to v5 than v8). Stage 1 was `a2bd5ef`: signals widget (Option K) live, stat row, rail reorder, tagline on its own line. Remaining: onboarding hatch ceremony, egg loader system, settle widget (needs BL-67). | Clickable final-product mock (approved pending Dino's click-through): slim one-line masthead, stat strip, Signal Board as an EGG PIE (Dino's call, 2026-08-01: ten repeated eggs read as wallpaper) — one radial chart, ten fixed wedges filling outward with signal, dashed shells for untouched topics, single egg at the hub, provenance detail panel beside it, no nested sidebar scrolling, decision-prompt card (BL-67's surface), grand hatch onboarding v9: a FOUR-SCENE ceremony (answers absorbed into the egg -> seven conversations fan out -> brief seeds itself -> shell cracks & hatches), v15 CONTINUOUS motion (Dino: 'shouldn't feel like a slideshow'): every entrance has a matching exit (chips absorb INTO the egg as the conversations fan out, cards drift outward, brief rows lift away) so nothing vanishes at a boundary; scenes overlap ~800ms; the egg never restarts its motion — nested scale/float/tilt wrappers, scale transitions 0.94→1.08 across the whole 15s as one unbroken arc; cracks draw by transition; ambient motes drift the entire run; titles/captions cross-drift with blur; the 4 dots keep a travelling pill. Always skippable, egg loading system (thinking/busy/hatching/empty). Artifact v3 (2026-08-01): rebuilt as the FINAL UI — verbatim tokens.css, embedded app typefaces (Geist/Geist Mono/Instrument Serif), full app shell (rail + topbar), v8 FINAL layout per Dino: conversations on the FAR LEFT (vertical list beside the icon rail), then the dashboard pair to its right as a two-column grid (stats 2x2 + settle widget + brief | signals egg pie + memory + artifacts); conversations keep their original vertical list as the RIGHT rail. Settle widget = questions' home until dedicated. Min-content overflow fixed at every grid level. v7 refinements: tagline moves to its own quiet second line under the idea name (both topbars); chat deck regains its collapse header and the scrollbar slims to 4px. v16: the COLLAPSED state reads as a closed drawer, not a divider — filled surface + border, chat icon, 'HIDE'/'SHOW' verb, and when closed it names where you are ("7 CHATS · YOU'RE IN HOW IT MAKES MONEY") beside 7 coverage pips (green=covered, amber=partial, ringed=current). claude.ai/code/artifact/d6d9d2c1-a4cf-4ec9-a4bb-ba6cfbeae0fa — v10 adds a Signal-options screen: EIGHT widget variants side by side (A egg pie / B signal bars / C heat carton / D incubation meter / E radar silhouette / F signal feed / G equalizer / H coverage ring / **I the sunrise — A x B hybrid: radial bars around the egg, counts at the tips, dashed stubs for zeros, strongest-first legend** / **J the nest + K egg markers — Dino's direction: B's bars leading, small dots back, egg holding the total, provenance quote beneath**). **v14: K CHOSEN and worn by the hub.** Bar semantics defined: fills toward 4 memories = 'topic properly explored' (ABSOLUTE, never relative — one strong topic must never shrink the others); count keeps climbing past full. Bottom line rewritten from data-speak to a sentence: 'Latest — pricing / "$1-5 per member…" / your words, from How it makes money · 2h ago', hover-follows the row, empty topics show an invitation. Tab 6 documents anatomy + three life stages (day one / in progress / well developed) awaiting Dino's pick — hybrid allowed (e.g. D's sentence layer under A's pie). Break into build items once Dino approves |
| BL-68 | **Corruption detector: template-fragment signature** | S | todo | Re-test 2026-08-01: "That{proven demand} +cohort locks it in" threaded all four BL-65 regexes — one brace pair, fluent prose. Replies never legitimately contain `{…}` (plain-text-only formatting), so the detector is cheap and precise. Known residual BELOW the detection floor: character-level word-mangling ("positing", "premury") — watch, don't regex-chase. Both instances followed data-dense founder answers |
| BL-69 | **Settled-vs-badge: go mechanical — inject computed chat status** | S | todo | Third strike for prompt-tuning this class (BL-60 → BL-63 → re-test failed twice: "all pinned down" at 2/3, enumerating intents it believed resolved). Per the sprint's own stop-rule: re-plan. Route computes unresolved required intents and injects "CHAT STATUS: 2/3 required resolved — missing: X" into context — the model can't honestly declare closure over the ledger's literal state |
| BL-70 | **Flag speculative competitors as unverified** | S | todo | Re-test: "Territory Foods" introduced with founder-fact confidence, never reconciled (correctly never reached memory — BL-57 held). The market prompt asks for real competitor names by design; the miss is calibration: world-knowledge suggestions should read as "worth checking", not scenery |
| BL-67 | **Decision prompts — the co-founder initiates** | M | doing | From Dino 2026-08-01. One-way prompts from the idea's memory ("the conscience"): a single card that asks the one thing worth settling, founder taps or types an answer, no rebuttal loop. Tier 1 (mechanical, zero model calls): unresolved required intents + brief open questions, answer writes an intent-tagged memory — coverage/brief machinery reused whole. Tier 2 (conscience pass, one small call): pick the highest-leverage question from assumptions to test, contradictions, `question`-kind memories, zero-signal topics. Escape hatch: "take it to the chat" deep-link. Load-bearing constraint: **never nagging** — one at a time, dismissible, "not now" sticks. Pairs with BL-61 (multi-select). **Tier 1 SHIPPED `bb8b39b`** 2026-08-01: hub asks the first unresolved required intent, answer writes an intent-tagged memory via POST /api/settle (ownership + intent-belongs-to-arc guarded), coverage/signals move, 'Not now' dismisses. **Tier 2 shape** (agreed with Dino 2026-08-01): one small model call reads the memory and picks the single highest-leverage question, and the question CHANGES REGISTER as the idea matures — unresolved intents early, then untested assumptions ('you're assuming nobody's built this — cheapest way to check?'), then cross-chat tensions ('brand says warm/communal, pricing says $1-5 a head — chipping in or a toll?'), then silent topics, then post-gate sharpening ('which single v1 feature would prove it?'). Options are the EVIDENCE it read your idea — drawn from the founder's own material, never generic. Constraints: cache the question until memory actually changes (one call per meaningful update, not per render), and answering must still write an intent-tagged memory — a prompt that doesn't feed the ledger is decoration. |
