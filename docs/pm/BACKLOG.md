...
| BL-72 | **Idea Hub redesign — per the vision artifact** | L | todo | Clickable final-product mock (approved pending Dino's click-through): slim one-line masthead, stat strip, Signal Board with per-topic provenance popovers, no nested sidebar scrolling, decision-prompt card (BL-67's surface), grand hatch onboarding (~15s ceremony, rotating quotes, always skippable), egg loading system (thinking/busy/hatching/empty). Artifact: claude.ai/code/artifact/d6d9d2c1-a4cf-4ec9-a4bb-ba6cfbeae0fa — break into build items once direction is approved |
