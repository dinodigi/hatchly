...
| BL-72 | **Idea Hub redesign — per the vision artifact** | L | todo | Clickable final-product mock (approved pending Dino's click-through): slim one-line masthead, stat strip, Signal Board as an EGG PIE (Dino's call, 2026-08-01: ten repeated eggs read as wallpaper) — one radial chart, ten fixed wedges filling outward with signal, dashed shells for untouched topics, single egg at the hub, provenance detail panel beside it, no nested sidebar scrolling, decision-prompt card (BL-67's surface), grand hatch onboarding (~15s ceremony, rotating quotes, always skippable), egg loading system (thinking/busy/hatching/empty). Artifact v3 (2026-08-01): rebuilt as the FINAL UI — verbatim tokens.css, embedded app typefaces (Geist/Geist Mono/Instrument Serif), full app shell (rail + topbar), v4 layout per Dino: CONVERSATIONS deck is the first line (7 rich cards, up-next flagged); signals compacted into the top card of the right rail (label-free egg pie + pinned detail + legend); settle card + 2x2 stats on the left. claude.ai/code/artifact/d6d9d2c1-a4cf-4ec9-a4bb-ba6cfbeae0fa — break into build items once Dino approves |
