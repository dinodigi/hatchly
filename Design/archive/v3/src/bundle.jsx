      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"36px 36px 100px" }}>
        ...
        <div className="row gap28" style={{ alignItems:"flex-start", gap:32 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            {tab==="security" && <SecurityPane onLogout={onLogout}/>}
          </div>
        </div>
      </div>
    </div>
...
function SecurityPane({ onLogout }) {
  return (
    <div>
...
function App() {
  ...
  useEffect(()=>{ window.__nav = (r)=>{ if(!loggedIn){ localStorage.setItem("hatchly-auth","1"); setLoggedIn(true); } go(r); }; window.__setTheme = setTheme; }, [loggedIn]);
