...
3. Update the brief when this turn justified it. Refine existing text freely as understanding sharpens; keep each section tight (one or two sentences; features/questions are short single lines). A section starting "Draft:" is an onboarding placeholder, NOT a filled section — replace it with confirmed, conversation-grade text as soon as you have it (drop the "Draft:" prefix; never write one yourself). Keep the brief PRODUCT-level: implementation details (auth providers, stacks, integrations) are memories with topic "tech" or "product", not brief features — unless they ARE the product.
