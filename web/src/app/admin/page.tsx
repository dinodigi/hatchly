...
/* Admin & moderation console (M6b) — the operator's view of the economy.
   Deliberately NOT generic CRUD: Pluggie's own admin already does raw data.
...
export default async function AdminPage() {
  ...
  const [econ, collusion, reports, listings, audit] = await Promise.all([
    economyStats(),
    collusionScan(),
    ...
    <div className="scrollarea">
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "34px 28px 100px" }}>
        ...
        <p className="muted" style={{ fontSize: 14.5, margin: "0 0 26px" }}>
          Money supply, funnelling signals, and the moderation queue. Every action here is logged.
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12, marginBottom: 14 }}>
          ...
          <Stat label="Minted" hint="grants + daily claims" tone="var(--success-text)">
            +{econ.minted.toLocaleString()}
          </Stat>
          <Stat label="Burned" hint="taxes + spotlight + unlocks" tone="var(--danger-text)">
            −{econ.burned.toLocaleString()}
          </Stat>
          <Stat
            label="Mint : burn"
            hint={
              econ.inflation === null
                ? "no sink throughput yet"
                : econ.inflation > 1
                  ? "sinks are not keeping up — supply is inflating"
                  : "sinks are absorbing supply"
            }
          ...
          <Stat
            ...
            hint={
              econ.drift === 0
                ? "sum(ledger) == sum(wallets) — the invariant holds"
                : "wallet balances disagree with the ledger — investigate"
            }
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 30 }}>
          <Card style={{ padding: 0, overflow: "hidden" }}>
            <div style={{ padding: "13px 18px", borderBottom: "1px solid var(--border)", fontWeight: 600, fontSize: 13.5 }}>
              Ledger by type
            </div>
            {econ.byType.map((r) => {
              // The tax rows are informational zeroes — the burn lives in the
              // invest_out/invest_in gap. Say so, or the table reads as "no sink".
              ...
              return (
                <div key={r.type} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 18px", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <span
                    className="mono"
                    style={{ fontSize: 13, fontWeight: 600, width: 90, textAlign: "right", color: informational ? "var(--text-muted)" : r.sum >= 0 ? "var(--success-text)" : "var(--text-primary)" }}
                  >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 18px", background: "var(--surface)" }}>
              ...
              <span className="mono" style={{ fontSize: 13, fontWeight: 600, width: 90, textAlign: "right", color: "var(--danger-text)" }}>
                −{econ.taxBurn.toLocaleString()}
              </span>
            </div>
          </Card>
        ...
        <SectionLabel style={{ marginBottom: 12 }}>
          Funnelling signals · {collusion.scanned} stakes scanned
        </SectionLabel>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 30 }}>
          <Card style={{ padding: 0, overflow: "hidden" }}>
            ...
            {collusion.pairs.length === 0 && (
              <p className="faint" style={{ padding: "16px 18px", margin: 0, fontSize: 12.5, fontStyle: "italic" }}>
                No two accounts are backing each other. Clean.
              </p>
            )}
            {collusion.pairs.slice(0, 8).map((p, i) => (
              <div key={`${i}-${p.a}-${p.b}`} style={{ padding: "11px 18px", borderBottom: "1px solid var(--border)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                  ...
                  <span className="spacer" style={{ flex: 1 }} />
                  {p.symmetry >= 0.8 && (
                    <span className="badge" style={{ fontSize: 9, background: "var(--danger-soft)", color: "var(--danger-text)" }}>
                      wash
                    </span>
                  )}
                </div>
          ...
          <Card style={{ padding: 0, overflow: "hidden" }}>
            ...
            {collusion.concentrated.length === 0 && (
              <p className="faint" style={{ padding: "16px 18px", margin: 0, fontSize: 12.5, fontStyle: "italic" }}>
                No founder is bankrolled by a single source.
              </p>
            )}
            {collusion.concentrated.slice(0, 8).map((c, i) => (
              <div key={`${i}-${c.founder}`} style={{ padding: "11px 18px", borderBottom: "1px solid var(--border)" }}>
                ...
                <div className="faint" style={{ fontSize: 11.5 }}>
                  {pct(c.share)} of {c.totalIn.toLocaleString()} from <b style={{ color: "var(--text-primary)" }}>{c.topBacker}</b>
                </div>
              </div>
            ))}
          </Card>
            ...
            <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 18px", borderBottom: "1px solid var(--border)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13.5, fontWeight: 500 }}>
                  {r.data.target_kind} · <span className="mono faint" style={{ fontSize: 11.5 }}>{r.data.target_id.slice(0, 8)}</span>
                </div>
            ...
            <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "11px 18px", borderBottom: "1px solid var(--border)" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {l.data.status === "hidden" && (
                    <span className="badge" style={{ fontSize: 9, background: "var(--danger-soft)", color: "var(--danger-text)" }}>hidden</span>
                  )}
                </div>
